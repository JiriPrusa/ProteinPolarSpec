...
from dipoleReporter import DipoleReporter
from chainSelector import ChainSelector
...
group.add_argument('-ch' , '--chain', dest='chain', default=None,
                   metavar='CHAR', help='''Letter of the chain you want to print
                   to dipole reporter and trajectory''')
group.add_argument('-d' , '--dip', dest='dip', default='dip.csv',
                   metavar='FILE', help='''Where to print dipole moments.''')                   
group.add_argument('-x', '--trajectory', dest='trajectory', default='traj.dcd',
                   metavar='FILE', help='''DCD format trajectory to generate.
...
group.add_argument('--interval', dest='interval', default=500, metavar='INT',
                   ...
                   type=int)
group.add_argument('-o', '--output', dest='out_state', default='output.xml',
                   metavar='FILE', help='''Xml serialized file with positions and velocities''')
...
for force in system.getForces():
    if isinstance(force, mm.AmoebaVdwForce):
        ...
        force.setMutualInducedTargetEpsilon(1e-5)
...
if opt.chain is not None:
    ch = ChainSelector(pdb.topology)
    proteinIndexList = ch.getIndexListForChain(opt.chain)
    print('Chain %s with %i elements selected for output. Warning chain letter must not correspond to letter in pdb file!!!' % (opt.chain, len(proteinIndexList)))
...
sim.reporters.append(
        DipoleReporter(opt.dip, opt.interval, totalDipole=True, inducedDipoles=True, makeSum=True, step=True, separator='\t', indexList=proteinIndexList)
)
sim.reporters.append(
        DCDReporter(opt.trajectory, opt.interval, proteinIndexList)
)

if opt.checkpoint is not None:
    sim.reporters.append(
            app.CheckpointReporter(opt.checkpoint, opt.interval*1000)
    )
...
with open(opt.out_state, 'w') as f:
    f.write(
